...
contract TOKEN721 is ERC165, IERC721 {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(to != owner, "ERC721 ERROR: Destination addres must be different");
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender), "ERC721 ERROR: You are not the owner you do not have permissions");
        _approve(to, tokenId);
    }
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != msg.sender, "ERC721 ERROR: Operator addres must be different");
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) internal virtual {
        ...
        require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721 ERROR: you are not the owner o no perms");
    } 
    ...
    function _safeMint(address to, uint256 tokenId, bytes memory _data) public {
        _mint(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721 ERROR: trnasfer to non ERC721Receiver implementer");
    }
    
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "ERC721 ERROR: mint to the zero address");
        require(!_exists(tokenId), "ERC721 ERROR: token alredy minted");
